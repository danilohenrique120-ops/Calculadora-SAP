...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({
  ...
  // Password modal for Operator Ranking
  const [isPasswordModalOpen, setIsPasswordModalOpen] = useState(false);
  const [isOperatorUnlocked, setIsOperatorUnlocked] = useState(false);
  ...
  const kpis = useMemo(() => {
    ...
    const avgVariancePercent = totalStdMin > 0 ? ((totalRealMin - totalStdMin) / totalStdMin) * 100 : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl space-y-4 relative">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            {isOperatorUnlocked ? (
              <div className="flex items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setIsOperatorUnlocked(false)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-500/15 hover:bg-rose-500/25 text-rose-300 hover:text-white text-xs font-semibold font-mono border border-rose-500/30 transition cursor-pointer shadow-sm"
                  title="Bloquear visualização desta seção novamente"
                >
          ...
          {isOperatorUnlocked ? (
            ...
            <div className="py-8 px-4 rounded-xl bg-slate-950/70 border border-slate-800/80 flex flex-col items-center justify-center text-center">
              <div className="w-12 h-12 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center text-cyan-400 mb-3 shadow-lg shadow-cyan-950/50">
                <Lock className="w-6 h-6" />
              </div>
              <h5 className="text-sm font-bold text-white mb-1">
                Visualização Protegida por Senha
              </h5>
              <p className="text-xs text-slate-400 max-w-md mb-4 leading-relaxed">
                Os dados individuais de performance, bateladas e aderência por operador são confidenciais e restritos à gestão.
              </p>
              <button
                type="button"
                onClick={() => setIsPasswordModalOpen(true)}
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-semibold text-xs transition shadow-lg shadow-cyan-950/40 cursor-pointer"
              >
        ...
        {/* Modal de Senha para Desbloqueio do Ranking de Operadores */}
        <PasswordModal
          isOpen={isPasswordModalOpen}
          onClose={() => setIsPasswordModalOpen(false)}
          onSuccess={() => {
            setIsOperatorUnlocked(true);
            setIsPasswordModalOpen(false);
            if (onAuthenticateAdmin) {
              onAuthenticateAdmin();
            }
          }}
          targetTitle="Performance & Aderência por Operador"
        />
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {/* HH Card */}
            {(() => {
              const hhPercent = costDriversData.totalHhStd > 0 ? ((costDriversData.totalHhReal - costDriversData.totalHhStd) / costDriversData.totalHhStd) * 100 : (costDriversData.count === 0 ? 0 : 100);
              return (
            ...
            {/* HM Card */}
            {(() => {
              const hmPercent = costDriversData.totalHmStd > 0 ? ((costDriversData.totalHmReal - costDriversData.totalHmStd) / costDriversData.totalHmStd) * 100 : (costDriversData.count === 0 ? 0 : 100);
              return (
            ...
            {/* GGF Card */}
            {(() => {
              const ggfPercent = costDriversData.totalGgfStd > 0 ? ((costDriversData.totalGgfReal - costDriversData.totalGgfStd) / costDriversData.totalGgfStd) * 100 : (costDriversData.count === 0 ? 0 : 100);
              return (
