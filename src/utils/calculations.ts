...
/**
 * Evaluates the status ('ok' | 'warning' | 'critical') based on variance in minutes and deviation percentage
 * using the configured thresholds (Verde: OK/No Prazo, Amarelo: Atenção, Vermelho: Desvio Crítico).
 * 
 * Desvio (Real - Standard):
 * - variancePercent <= 0% (ou varianceMin <= 0): Economizou tempo ou cumpriu o standard -> 🟢 Verde (OK)
 * - variancePercent > 0%: Passou do standard
 *   - Se atraso <= tolerância verde (ex: 0 min): 🟢 Verde
 *   - Se atraso <= limite de atenção (ex: até +15% ou <= 30 min): 🟡 Amarelo
 *   - Se atraso > limite de atenção: 🔴 Vermelho
 */
export function evaluateVarianceStatus(
  varianceMin: number, // Real - Standard
  variancePercent: number, // ((Real - Standard) / Standard) * 100
  thresholds?: VarianceThresholdConfig
): StageStatus {
  const config = thresholds || getStoredVarianceThresholds();
  const allowedDelayPct = Math.max(0, 100 - (config.yellowMinPercent ?? 85)); // Default: até +15% de atraso para amarelo

  if (config.evaluationMode === 'minutes') {
    // Avaliação direta por minutos de atraso (Real - Standard)
    if (varianceMin <= (config.greenMaxDelayMin ?? 0)) {
      return 'ok';
    }
    if (varianceMin <= (config.yellowMaxDelayMin ?? 30)) {
      return 'warning';
    }
    return 'critical';
  } else if (config.evaluationMode === 'hybrid') {
    // Modo Híbrido: Se dentro da tolerância de minutos OU se cumpriu o standard (variação <= 0%)
    if (varianceMin <= (config.greenMaxDelayMin ?? 0) || variancePercent <= 0) {
      return 'ok';
    }
    if (varianceMin <= (config.yellowMaxDelayMin ?? 30) || variancePercent <= allowedDelayPct) {
      return 'warning';
    }
    return 'critical';
  } else {
    // Modo Percentual de Desvio (com respeito à tolerância em minutos)
    const toleranceMin = config.greenMaxDelayMin ?? 0;
    if (varianceMin <= toleranceMin || variancePercent <= 0) {
      return 'ok';
    }
    if (variancePercent <= allowedDelayPct || varianceMin <= (config.yellowMaxDelayMin ?? 30)) {
      return 'warning';
    }
    return 'critical';
  }
...
export function calcCombinedAbastecimentoPreparoMetrics(
  ...
  // Variance %: ((Real - Standard) / Standard) * 100
  const hhVarPct = stdHh > 0 ? ((realHh - stdHh) / stdHh) * 100 : (realHh === 0 ? 0 : 100);
  const hmVarPct = stdHm > 0 ? ((realHm - stdHm) / stdHm) * 100 : (realHm === 0 ? 0 : 100);
  const ggfVarPct = stdGgf > 0 ? ((realGgf - stdGgf) / stdGgf) * 100 : (realGgf === 0 ? 0 : 100);
  ...
  switch (status) {
    case 'ok':
      return {
        ...
        label: 'No Prazo',
        subLabel: 'Desvio <= 0% (ou tolerância)',
      };
    case 'warning':
      return {
